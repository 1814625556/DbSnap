...
namespace SqlliteTest
{
    public class SqlLiteHelp
    {
        ...
        {
            string path = @"C:\sqlite\sqlite-tools-win32-x86-3240000\chenchang.db";
            SQLiteConnection cn = new SQLiteConnection("data source=" + path);
            if (cn.State != System.Data.ConnectionState.Open)
            {
                cn.Open();
    ...
    public abstract class DbHelperSQLite
    {
        ...
        /// </summary>
        /// <param name="SQLStringList">执行的sql语句</param>
        public static bool ExecuteInsertSqlTran(List<string> SQLStringList)
        {
            using (SQLiteConnection conn = new SQLiteConnection(connectionString))
            {
                ...
                try
                {
                    ...
                    tx.Commit();
                    sw.Stop();
                    //loggerDebug.Debug("批量插入SQLite:" + SQLStringList.Count + "条数据，所占时间:" + sw.ElapsedMilliseconds.ToString() + "毫秒");
                    return true;
                }
                catch (System.Data.SQLite.SQLiteException E)
                {
